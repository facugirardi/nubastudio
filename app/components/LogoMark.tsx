"use client";

import { useEffect, useRef } from "react";
import { gsap } from "gsap";

const PATH_D =
  "M3.00897 14.3017C2.20703 13.3954 1.62112 12.3191 1.29535 11.1537C0.969576 9.98825 0.912436 8.7641 1.12823 7.5734C1.34402 6.3827 1.82712 5.25646 2.54115 4.27948C3.25519 3.3025 4.18156 2.50023 5.25052 1.93308C6.31948 1.36592 7.50317 1.04866 8.71249 1.00517C9.92181 0.961679 11.1252 1.1931 12.2322 1.68201C13.3391 2.17091 14.3207 2.90456 15.1031 3.82773C15.8855 4.7509 16.4482 5.83952 16.749 7.01165H18.009C19.0718 7.01082 20.1072 7.34868 20.965 7.97621C21.8228 8.60374 22.4584 9.48828 22.7794 10.5015C23.1004 11.5147 23.0902 12.6038 22.7503 13.6108C22.4103 14.6178 21.7689 15.5213 20.889 16.1017C20.009 16.682 13.009 16.5117 13.009 16.5117";

export default function LogoMark({
  play = true,
  size = 30,
}: {
  play?: boolean;
  size?: number;
}) {
  const pathRef = useRef<SVGPathElement>(null);
  const drawRef = useRef<gsap.core.Tween | null>(null);

  useEffect(() => {
    if (!play) return;
    const path = pathRef.current;
    if (!path) return;

    const L = path.getTotalLength();
    const reduce = window.matchMedia("(prefers-reduced-motion: reduce)").matches;

    gsap.set(path, { strokeDasharray: L, strokeDashoffset: reduce ? 0 : L });
    if (reduce) return;

    // La nube se traza sola una vez y queda quieta.
    const draw = gsap.to(path, {
      strokeDashoffset: 0,
      duration: 2.4,
      ease: "power2.inOut",
      delay: 0.95,
    });
    drawRef.current = draw;

    return () => {
      draw.kill();
      drawRef.current = null;
    };
  }, [play]);

  const onEnter = () => {
    const path = pathRef.current;
    if (!path) return;
    // Redibujo rápido bajo demanda; solo si la entrada ya terminó.
    if (drawRef.current && drawRef.current.isActive()) return;
    const L = path.getTotalLength();
    gsap.fromTo(
      path,
      { strokeDashoffset: L },
      { strokeDashoffset: 0, duration: 1.3, ease: "power2.out", overwrite: true }
    );
  };

  return (
    <svg
      width={size}
      height={(size * 18) / 25}
      viewBox="0 0 25 18"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      role="img"
      aria-label="Nuba Studio"
      onMouseEnter={onEnter}
      style={{ display: "block", overflow: "visible" }}
    >
      <path
        ref={pathRef}
        d={PATH_D}
        stroke="#ffffff"
        strokeWidth={1.5}
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  );
}
